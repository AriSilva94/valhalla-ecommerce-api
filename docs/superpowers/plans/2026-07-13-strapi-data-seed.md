# Strapi Data Seed Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Populate every Strapi content-type with the real hardcoded content from the frontend project (products, categories, brands, tag, homepage sections, site settings, FAQ, policies) and grant public read access, per `docs/superpowers/specs/2026-07-13-strapi-data-seed-design.md`.

**Architecture:** Rewrite the existing `scripts/seed.js` (already wired to `npm run seed:example`, currently dead code referencing deleted blog content-types) to seed the e-commerce content model in-process via Strapi's Document Service API (`strapi.documents(uid).create(...)`) and grant public permissions via `strapi.query('plugin::users-permissions.permission')` — no HTTP calls, no API token, matching the file's pre-existing convention. Run it once against the live dev Strapi instance, then verify via public (unauthenticated) REST GET requests.

**Tech Stack:** Strapi 5.50.1, Node (script run via `node ./scripts/seed.js`), SQLite dev DB at `.tmp/data.db`.

## Global Constraints

- Project root: `C:\Users\ariov\Desktop\projetos\valhalla-ecommerce-api`. Not a git repository — no commit steps.
- Do **not** delete `.tmp/data.db` (holds the real admin user).
- All literal seed content below is transcribed verbatim from `C:\Users\ariov\Desktop\projetos\valhalla-ecommerce\app\lib\data.ts`, `app\components\Store.tsx`, and `app\layout.tsx` — do not paraphrase or "improve" the copy.
- Media fields (`logo`, `image`, `mainImage`, `gallery`, `aboutImage`, `shareImage`, variant `image`) are left empty — no image assets exist yet.
- `Product` and `ProductVariant` have `draftAndPublish: true`; every `documents(...).create()` call for these two must pass `status: 'published'` or the records won't be publicly visible. All other content-types have `draftAndPublish: false` — no `status` param needed/applicable.
- Strapi v5 Document Service relation inputs: a to-one relation is the related record's `documentId` string; a to-many relation is an array of `documentId` strings.
- The seed is guarded by the file's existing `isFirstRun()` pattern (a `strapi.store()` flag) — running `npm run seed:example` a second time must be a safe no-op, not a duplicate-data error.

---

### Task 1: Rewrite `scripts/seed.js` with the full e-commerce seed

**Files:**
- Modify: `scripts/seed.js` (replace entire contents)
- Delete: `data/data.json` (blog-template leftover; nothing will reference it once this task is done)

**Interfaces:**
- Consumes: the 9 Strapi content-type UIDs built in the earlier content-model plan (`api::brand.brand`, `api::category.category`, `api::tag.tag`, `api::product.product`, `api::product-variant.product-variant`, `api::homepage.homepage`, `api::site-setting.site-setting`, `api::faq.faq`, `api::policy.policy`) and their exact attribute names.
- Produces: `npm run seed:example` becomes a working one-shot seed command (exercised in Task 2).

- [ ] **Step 1: Confirm current file is the dead blog-template version (sanity check before overwriting)**

```bash
cd "C:\Users\ariov\Desktop\projetos\valhalla-ecommerce-api"
grep -c "data/data.json" scripts/seed.js
```
Expected: `1` (confirms this is still the untouched template file being replaced, not something already customized).

- [ ] **Step 2: Replace `scripts/seed.js` with the following complete content**
...
async function seedBrands() {
  ...
  for (const name of BRANDS) {
    map[name] = await strapi.documents('api::brand.brand').create({ data: { name } });
  }
  return map;
}

async function seedTag() {
  return strapi.documents('api::tag.tag').create({ data: { name: 'Novo' } });
}
...
async function seedPolicies() {
  for (const p of POLICIES) {
    await strapi.documents('api::policy.policy').create({ data: { title: p.q, body: p.a } });
  }
}
...
- [ ] **Step 3: Delete the dead blog-template data file**
...
- [ ] **Step 4: Syntax-check the rewritten script (does not run it against Strapi yet)**
...
- [ ] **Step 5: Confirm data completeness by counting literal entries**

```bash
cd "C:\Users\ariov\Desktop\projetos\valhalla-ecommerce-api"
node -e "
const fs = require('fs');
const src = fs.readFileSync('scripts/seed.js', 'utf-8');
const productCount = (src.match(/id: '[a-z0-9]+', name:/g) || []).length;
console.log('products:', productCount);
console.log('categories:', (src.match(/id: '(smartphones|notebooks|tablets|smartwatches|audio|acessorios|games)'/g) || []).length);
console.log('brands array present:', /const BRANDS = \[/.test(src));
console.log('faqs:', (src.match(/const FAQS = \[[\s\S]*?\];/)[0].match(/\{ q:/g) || []).length);
console.log('policies:', (src.match(/const POLICIES = \[[\s\S]*?\];/)[0].match(/\{ q:/g) || []).length);
"
```
Expected: `products: 18`, `categories: 7`, `brands array present: true`, `faqs: 6`, `policies: 4`.

---

### Task 2: Run the seed and verify via public REST API

**Files:** none (execution + verification only).

**Interfaces:**
- Consumes: the rewritten `scripts/seed.js` from Task 1, and the live Strapi dev server.
- Produces: populated `.tmp/data.db`, publicly readable via `http://localhost:1337/api/*`.

- [ ] **Step 1: Make sure Strapi is not already running (the seed script boots its own in-process instance and would conflict with a running dev server on the same SQLite file)**
...
```
Expected output includes: `Seeding Valhalla e-commerce data...`, then a `Seeded 7 categories, 9 brands, 1 tag, 18 products, N product variants, homepage, site settings, 6 FAQs, 4 policies.` line (N will be some number around 80-100 — record the exact value from this run), then `Done.`. No stack traces. If a relation-input or field-validation error appears, read it, fix the corresponding line in `scripts/seed.js` (most likely cause: a relation expects a `documentId` string/array in a different shape than written — consult the error message's field name), and re-run from Step 1 (the `isFirstRun()` guard means a failed run does NOT mark itself as done, so re-running after a fix is safe and picks up where the code now is).

- [ ] **Step 3: Start the Strapi dev server for the verification requests**
...
- [ ] **Step 4: Verify public counts with no Authorization header**
...
(async () => {
  const endpoints = {
    brands: 9, categories: 7, tags: 1, products: 18,
    'product-variants': null, // exact count recorded from Task 2 Step 2's log line
    homepage: null, 'site-setting': null, faqs: 6, policies: 4,
  };
  for (const [path, expected] of Object.entries(endpoints)) {
    ...
    console.log(path, 'status=' + res.status, 'count=' + count, expected !== null ? ('expected=' + expected) : '');
  }
})();
"
```
Expected: every endpoint returns `status=200`. `brands`→9, `categories`→7, `tags`→1, `products`→18, `faqs`→6, `policies`→4 exactly; `product-variants` matches the N logged in Task 2 Step 2; `homepage`/`site-setting` each return `count=1` (single-types return a single object in `data`, not an array).

- [ ] **Step 5: Spot-check one product's relations resolve**

```bash
cd "C:\Users\ariov\Desktop\projetos\valhalla-ecommerce-api"
node -e "
(async () => {
  const res = await fetch('http://localhost:1337/api/products?filters[name][\$eq]=Galaxy S25 Ultra&populate=*');
  const body = await res.json();
  const p = body.data[0];
  console.log('name:', p.name);
  console.log('brand:', p.brand && p.brand.name);
  console.log('category:', p.category && p.category.name);
  console.log('tags:', (p.tags || []).map(t => t.name));
  console.log('variants count:', (p.variants || []).length);
  console.log('specs count:', (p.specs || []).length);
})();
"
```
Expected: `name: Galaxy S25 Ultra`, `brand: Samsung`, `category: Smartphones`, `tags: []` (S25U's `tag` is `null` in the source, not `"NOVO"` — no tag expected here), `variants count: 9` (3 colors × 3 configs), `specs count: 4`.
...
- **Spec coverage:** every entity in the design doc (Brand, Category, Tag, Product, ProductVariant, Homepage, SiteSettings, Faq, Policy) has seed logic in Task 1 and a verification check in Task 2. The cartesian variant generation, the SKU scheme, the product-level `stock: 'out'` → all-variants-unavailable override, the dropped footer "Categorias"/"404 demo" links, and the public-permissions-via-internal-API mechanism all match the spec.
- **Placeholder scan:** no TBD/TODO. The one intentionally-unresolved number (`product-variants` expected count in Task 2 Step 4) is explicitly called out as "read from Task 2 Step 2's own log output," not a vague placeholder — the script computes and prints the real number itself.
- **Type consistency:** `productsById`/`categoriesById`/`brandsByName` map keys (Task 1) match exactly what `seedProducts`/`seedProductVariants` read; `HOMEPAGE_DATA`/`SITE_SETTINGS_DATA` object keys match the content-type attribute names from the content-model spec (`ctaLink`, `secondaryCtaLink`, `trustBadges`, `footerLinkColumns`, `aboutStats`, `defaultSeo`, etc.) exactly.
- **Corrected during planning:** the design doc's original `ctaLink: "/p/s25u"` guess used the frontend's internal short product code; this plan uses `/p/galaxy-s25-ultra` instead, matching the slug Strapi will actually auto-generate from the product's `name` (`uid` fields auto-slugify from their `targetField` when not explicitly supplied) — the short code has no meaning once real Strapi slugs exist.
