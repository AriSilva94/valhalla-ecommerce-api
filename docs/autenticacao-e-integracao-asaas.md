...
- callback permitido somente em \`https://<frontend>/api/auth/google/callback\`;
...
O teste inicial é somente leitura, por exemplo \`GET /customers?limit=1\`. A rota temporária interna do Strapi exige autenticação administrativa ou política interna explícita; não pode ser pública.
...
| \`ASAAS_API_URL\`, \`ASAAS_API_KEY\`, \`ASAAS_TIMEOUT_MS\`, \`ASAAS_USER_AGENT\` | Asaas |
