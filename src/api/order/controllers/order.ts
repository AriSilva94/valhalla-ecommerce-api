...
import { getIdempotencyResult, saveIdempotencyResult } from '../../../order/idempotency';
...
export default {
  async create(ctx: Context) {
    ...
    const cachedResponse = await getIdempotencyResult<{ ok: true; data: ReturnType<typeof serializeOrder> }>(redis, userId, idempotencyKey);
    if (cachedResponse) {
      ctx.status = 200;
    ...
    if (existingOrder) {
      ...
      if (isCompletedOrder(existingOrder)) {
        const response = { ok: true as const, data: serializeOrder(existingOrder) };
    ...
    const response = { ok: true as const, data: serializeOrder(processed) };
    await saveIdempotencyResult(redis, userId, idempotencyKey, response);
    ctx.status = 201;
    ctx.body = response;
  },
