...
const PRODUCT_PATCH: ViewPatch = {
  ...
  fields: {
    ...
    variantGroupLabel: {
      ...
      description: 'Ex.: "Tamanho", "Configuração". Aparece acima das opções na página do produto',
    },
    variants: {
      label: 'Variações',
      description:
    ...
    seo: {
      label: 'SEO (opcional)',
      description: 'Pode deixar vazio — é preenchido automaticamente com nome e descrição',
    },
  },
};
...
function isValidConfig(value: unknown): value is Record<string, any> {
  ...
  const validEditLayout = value.layouts.edit.every(
    (row: unknown) =>
      Array.isArray(row) &&
      row.every((cell: unknown) => isPlainObject(cell) && typeof cell.name === 'string')
  );
  const validListLayout = value.layouts.list.every((field: unknown) => typeof field === 'string');
...
function applyPatch(config: any, patch: ViewPatch): void {
  ...
  for (const [field, fieldPatch] of Object.entries(patch.fields)) {
    ...
    const editMetadata = isPlainObject(metadata.edit) ? metadata.edit : undefined;
    const listMetadata = isPlainObject(metadata.list) ? metadata.list : undefined;
    ...
    if (fieldPatch.hidden && Array.isArray(config.layouts?.edit)) {
      config.layouts.edit = config.layouts.edit
        .map((row: unknown) =>
          Array.isArray(row)
            ? row.filter((cell: unknown) => !isPlainObject(cell) || cell.name !== field)
            : row
        )
...
export async function seedAdminViews(strapi: Core.Strapi): Promise<void> {
  ...
  for (const [key, patch] of Object.entries(ALL_PATCHES)) {
    ...
    const parsedConfig = safeParseJson(row.value, () => {
      strapi.log.warn(`[admin-view-seed] config inválida, adiada: ${key} (JSON malformado)`);
    });
    ...
    if (!isValidConfig(parsedConfig.value)) {
      allApplied = false;
      strapi.log.warn(`[admin-view-seed] config inválida, adiada: ${key} (formato inesperado)`);
      continue;
    }
